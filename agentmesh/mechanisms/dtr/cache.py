...
Implements tool-result caching with projection-based delta extraction and SLM
query reformulation for the produce step of a tool→LLM dataflow.

Core idea: decompose a tool query into a *semantic base* (reusable from cache)
and a *semantic delta* (novel intent requiring incremental retrieval). DTR
serves the base immediately, fetches only the delta, and async-refreshes the
exact cache entry — eliminating REDUNDANCY in the produce stage.

Key formulas (Algorithm 1):
- Projection intensity: s_i = v_in · v_i  (cosine similarity for normalized vectors)
- Semantic delta:       r = v_in - s_max * v_max
...
- Confidence gating:    activate DTR only when s_max ≥ τ (default τ=0.8)
"""
...
@dataclass
class CacheResult:
    """Result of a DTR cache lookup."""
    hit: bool                                           # Whether DTR was activated
    projection_intensity: float = 0.0                   # s_max
    cached_entry: Optional[CacheEntry] = None           # Best-matching cache entry
    reuse_count: int = 0                                # ⌊s_max·n⌋ items reused (base)
    residual_count: int = 0                             # ⌈(1-s_max)·n⌉ items to fetch (delta)
    residual_vector: Optional[np.ndarray] = None        # r = v_in - s_max * v_max
    hotspot_tokens: List[str] = field(default_factory=list)  # Top-k delta tokens
    reformulated_query: Optional[str] = None            # SLM-generated differential query

    @property
    def residual_ratio(self) -> float:
        """Ratio of delta items to total retrieval depth."""
...
class HotspotTokenExtractor:
    """
    Identifies delta-characterized tokens from the query.
    ...
    SLM query reformulation of the delta.
    """
    ...
    def extract_hotspot_tokens(
        ...
        """
        Extract top-k hotspot tokens from query based on the delta direction.

        Args:
            query: Original query Q_in.
            residual_vector: Normalized delta vector r_hat.
...
class QueryReformulator:
    ...
    The reformulated query focuses on the semantic delta — intent that
...
class DTRCache:
    """
    Delta Tool Retrieval Cache.
    Implements the complete DTR pipeline (Algorithm 1):
    1. Embed incoming query as normalized vector v_in
    2. Compute projection intensity s_i = v_in · v_i for each cached query
    3. Confidence gating: s_max ≥ τ → DTR activation; s_max < τ → cold retrieval
    4. DTR activation:
       a) Reuse ⌊s_max·n⌋ items (the base) from best-matching cache entry
       b) Compute delta r = v_in - s_max * v_max
       c) Extract hotspot tokens via c_j = e_j · r_hat
       d) SLM reformulates differential query for the delta
       e) Fetch remaining ⌈(1-s_max)·n⌉ items (the delta) via reformulated query
    5. Asynchronous cache refresh: background full execution prevents drift

    Key parameters:
    - τ (confidence_threshold): default 0.8 per paper deployment
    - n (retrieval_depth): standard number of items per tool query
    - k (hotspot_k): top-k delta tokens, default 3
    """
    ...
    def __init__(
        ...
        """
        Initialize the DTR cache.

        Args:
            confidence_threshold: Semantic confidence threshold τ (default 0.8).
            retrieval_depth: Standard retrieval depth n (items per query).
            cache_size: Maximum number of entries in cache pool.
            hotspot_k: Number of hotspot tokens for delta analysis.
    ...
    def lookup(
        ...
        """
        DTR cache lookup implementing Algorithm 1.

        Args:
            query: Incoming tool query Q_in.
            tool_fn: Optional tool execution function for async cache refresh.

        Returns:
            CacheResult with base/delta allocation and reformulated query.
        """
        ...
        if s_max < self.confidence_threshold or best_entry is None:
            ...
            logger.debug(f"DTR Cold Retrieval: s_max={s_max:.3f} < τ={self.confidence_threshold}")
            return CacheResult(
                hit=False,
        ...
        # Step 4a: Item allocation (base vs. delta)
        ...
        # Step 4b: Compute semantic delta vector
        ...
        # Step 5: Schedule asynchronous cache refresh (background full execution)
        if tool_fn is not None:
            self._schedule_async_update(query, v_in, tool_fn)

        logger.debug(
            f"DTR Activation: s_max={s_max:.3f}, reuse={k_reuse}, "
            f"delta={k_residual}, hotspots={hotspot_tokens}"
        )
    ...
    def get_reused_items(
        ...
        """
        Get the reused items (the cached base) from a cache hit.
    ...
    def _schedule_async_update(
        ...
        """
        Schedule asynchronous cache refresh.

        Runs the full tool execution in background to refresh the cache
        with authentic results, preventing cumulative drift from delta
        approximation.
        """
        def _background_update():
            try:
                ...
                logger.debug(f"Async cache refresh completed for: {query[:50]}...")
            except Exception as e:
                logger.warning(f"Async cache refresh failed: {e}")

        thread = threading.Thread(target=_background_update, daemon=True)
        thread.start()

    def get_hit_rate(self) -> float:
        """Get DTR activation rate (fraction of lookups with s_max ≥ τ)."""
        total = self.stats["total_lookups"]
        return self.stats["dtr_activations"] / total if total > 0 else 0.0
...
def create_dtr_cache(
    ...
    **kwargs,
    ...
    """
    Create a DTR cache instance.

    Args:
        confidence_threshold: τ — semantic confidence gate (default 0.8).
        retrieval_depth: n — items per query (default 10).
        cache_size: Maximum cache pool size.
        hotspot_k: k — top-k delta tokens (default 3).
        embedding_model: Encoder model name.
        slm_model: SLM for query reformulation.

    Returns:
        Configured DTRCache instance.
    """
    return DTRCache(
