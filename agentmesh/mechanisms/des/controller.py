...
Implements adaptive streaming chunk-size control via Online Sensitivity
Estimation (OSE) to drive a dataflow to dynamic equilibrium (slack δ→0),
overlapping the produce/transfer/consume stages.

DES decomposes total latency:  T_total = L_start + L_work + L_stall
and controls the chunk size θ to minimize stall time L_stall (the BARRIER).

Two pipelines served:
  1. Tool-to-agent pipeline — DTR items streamed to the agent's LLM prefill
...
Key formulas:
  ...
  ρ_i      = S_{pro,i} - S_{con,i-1}       — Relative sensitivity ρ = Ŝ_pro − Ŝ_con
  Δθ_i     = -δ_i / ρ_i                    — Newton control step
...
Variables:
  ...
  δ_i     — stall time (slack / idle gap)
  θ_i     — chunk size (tokens)
...
class Observation:
    """
    Observation data point for the DES control loop.

    Captures timing information for a single chunk transmission:
    - chunk_size (θ_i): Chunk size in tokens
    ...
    """
    chunk_size: int          # θ_i — chunk size (tokens)
...
class Sensitivity:
    """
    Sensitivity estimates for DES control (OSE output).
...
class OnlineSensitivityEstimator:
    ...
    def __init__(self, window_size: int = 5):
        """
        Initialize the online sensitivity estimator.
...
class DESController:
    """
    Dynamic-Equilibrium Streaming Controller.

    Core control algorithm:
    1. Online Sensitivity Estimation (OSE) via OnlineSensitivityEstimator
    2. Polarized control based on ρ sign:
       - ρ > 0 → Production-Bound → increase θ (larger chunks)
       - ρ < 0 → Consumption-Bound → decrease θ (smaller chunks)
    3. Bounded adaptive chunk-size adjustment toward equilibrium (δ→0)

    Control law:
      Δθ_i     = -δ_i / ρ_i      (Newton step, ρ = Ŝ_pro − Ŝ_con)
      θ_{i+1}  = clip(θ_i + Δθ_i, θ_min, θ_max)
    """
    ...
    def __init__(
        ...
        """
        Initialize DES controller.

        Args:
            initial_theta: Initial chunk size in tokens.
            min_theta: Minimum allowed chunk size (θ_min).
            max_theta: Maximum allowed chunk size (θ_max).
        ...
        # Online sensitivity estimator
        self.observer = OnlineSensitivityEstimator(window_size=window_size)
    ...
    def compute_next_theta(
        ...
        """
        Compute optimal chunk size for next iteration.
        ...
        Returns:
            Recommended chunk size θ for next iteration.
        """
        self.step_count += 1
        ...
        logger.debug(f"DES: θ {old_theta} → {new_theta} (δ={slack_time:.2f}, reason={adjustment_reason})")
    ...
    def _compute_adjustment(
        ...
        Operational regimes:
          ρ > 0 → Production-Bound: δ > 0 → Δθ < 0 (shrink chunk)
          ρ < 0 → Consumption-Bound: δ < 0 → Δθ < 0 (shrink chunk)
...
def create_des_controller(
    ...
    """
    Create a DES controller instance.
    ...
    Returns:
        Configured DESController instance.
    """
    return DESController(
