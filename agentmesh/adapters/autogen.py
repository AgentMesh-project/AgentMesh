...
Provides integration with the Microsoft AutoGen multi-agent framework.
Wraps AutoGen agents with AgentMesh sidecars to enable DTR, BPP/BPA, and DES
...
class AutoGenAdapter:
    ...
    - DTR delta retrieval between agent conversations (the produce step)
    - BPP/BPA branch-parallel KV prefill for supervisor-worker topology
      (the consume step)
    - DES dynamic-equilibrium chunk streaming (stage overlap)
    ...
    def wrap_agent(self, agent: Any) -> WrappedAgent:
        """
        Wrap a single AutoGen agent with an AgentMesh sidecar.
    ...
    async def _intercept_generate(
        ...
        **kwargs
        ...
        """
        Intercept an agent's generate_reply to apply AgentMesh optimizations.

        Checks the DTR cache first, falls back to original generation on a miss.
        """
        if not messages:
        ...
        if cache_result and cache_result.hit and cache_result.residual_ratio < 0.2:
            ...
            logger.debug(f"DTR cache hit for agent {wrapped.agent_id}")
        ...
        if wrapped.sidecar.dtr_cache:
            wrapped.sidecar.dtr_cache.store(content, [response])
...
def create_autogen_adapter(
    ...
    **kwargs
    ...
    Args:
        dtr_enabled: Enable delta tool retrieval (DTR).
        bpp_enabled: Enable branch-parallel prefill (BPP).
        des_enabled: Enable dynamic-equilibrium streaming (DES).
    ...
    runtime = create_runtime(
        dtr_enabled=dtr_enabled,
        bpp_enabled=bpp_enabled,
